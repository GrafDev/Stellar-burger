  ...
  return (
    <li className={location.pathname === String(PATH_PROFILE_ORDERS) ? `${style.section__profile}` : `${style.section}`}>
      <div className={`${style.block} mt-6 mb-6`}>
        ...
        <p className={`${style.data} ${activeDefault} ${colorInactive}`}><FormattedDate date={new Date(createdAt)} /> i-GMT+3</p>
      </div>
      ...
      <div className={`${style.block} mb-6 mt-6`}>
        <ul className={`${style.block__card}`}>
          ...
          {(counterIngredient) ? (<div className={`${style.card__count} ${digitsMedium}`}>+{counterIngredient}</div>) : null}
        </ul>
